...
const inputPath = path.isAbsolute(inputFilePath) ? inputFilePath : path.resolve(process.cwd(), inputFilePath);
...
async function convertMdxToHtml() {
  ...
  const { data: frontmatter, content: contentWithoutFrontmatter } = matter(rawMdxContent);
  
  const processedMdxContent = await groupAndWrapStandaloneElements(contentWithoutFrontmatter);
